...
# update is main funtion we used object same or not by
# finding ditance bw frame 1 and frame 2
# when car goes 8th pixel and go 7th pixel we say that same object itself
...
# threshold we increase detect multiple same object and multiple ids
# and dec the multiple vehicle will get same id     diffrent videos for diffrent thershold
                if dist < 70:
                    ...
                    # CAPTURE FLAG 1 means capture the image of & flag 0 = oncle it is capture not again n again running
                    if (y < 235):
                        self.f[id] = 1

  # when distance not leeser then 70  the new object detected  and is would be given to it 
            # NEW OBJECT DETECTION  assigning a new id 
...
# 214 That number differs for different footages. 
# I calculated that number by taking the first car to go at around 50 km/hr.
